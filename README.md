...
<body translate="no" >
   ...
   <pre class="language-markup">
                <code>
                           <script type="text/plain"><amp-iframe width="200" height="1200"
    sandbox="allow-scripts allow-same-origin"
    layout="responsive"
    frameborder="0"
    src="https://cdn.rawgit.com/cenggini/warna/master/warna-webcensar.html">
</amp-iframe></script>
                </code>
            </pre>
    ...
    <script >
      /* http://prismjs.com/download.html?themes=prism&languages=markup+css+clike+javascript+abap+actionscript+ada+apacheconf+apl+applescript+asciidoc+aspnet+autoit+autohotkey+bash+basic+batch+c+brainfuck+bro+bison+csharp+cpp+coffeescript+ruby+css--extras+powershell+processing+prolog+properties+protobuf+puppet+pure+python+q+qore+r+jsx+reason+rest+rip+roboconf+crystal+rust+sas+sass+scss+scala+scheme+smalltalk+smarty+sql+stylus+swift+tcl+textile+twig+typescript+verilog+vhdl+vim+wiki+xojo+yaml&plugins=toolbar+previewer-base+previewer-angle+unescaped-markup+normalize-whitespace+keep-markup+copy-to-clipboard */
	...
	Prism.plugins.toolbar.registerButton('copy-to-clipboard', function (env) {
		...
		function registerClipboard() {
			...
			clip.on('success', function() {
				linkCopy.textContent = 'Copy Succses!';
		...
		function resetText() {
			setTimeout(function () {
				linkCopy.textContent = 'copy to clipboard';
			}, 5000);
		}
	});
})();
